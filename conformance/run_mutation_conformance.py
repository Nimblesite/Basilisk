...
"""Score Basilisk on the AST-PRESERVING MUTATED python/typing conformance suite.
...
The flow mirrors ``run_conformance.py`` and reimplements nothing:
  ...
  1. clone ``python/typing`` fresh (same ref plumbing as the pristine scorer),
...
def clone_suite(ref: str, suite_dir: Path, reuse: bool) -> Path:
    ...
    if not suite_dir.exists():
        subprocess.run(
            ["git", "clone", "--quiet", "https://github.com/python/typing", str(suite_dir)],
            check=True,
        )
        subprocess.run(
...
def run_harness(suite_dir: Path, binary: Path) -> Path:
    ...
    env = {"BASILISK_BIN": str(binary.resolve()), "PATH": "/usr/bin:/bin:/usr/local/bin"}
    import os
...
def main() -> int:
    ...
    parser.add_argument("--bin", required=True, type=Path, help="basilisk release binary")
    # ⚠️ python/typing@main removed the Basilisk adapter in c43d32e (typing#2330),
    # so `--only-run basilisk` is invalid at HEAD until it is restored. a490662
    # is the last commit whose OWN unmodified harness carries the official
    # BasiliskTypeChecker; move this default back to "main" the moment the
    # adapter returns upstream.
    parser.add_argument(
        "--ref",
        default="a490662",
        help="python/typing ref to score against (see adapter note above)",
    )
    parser.add_argument(
        "--suite-dir",
        type=Path,
    ...
    if rate < floor:
        print(
            ...
            f"formatting-dependent verdicts ([CHKARCH-CONFORMANCE-MODE]).",
            file=sys.stderr,
        )
        return 1
