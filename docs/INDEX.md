...
| [CHECKER-STUB-RESOLUTION-PLAN.md](plans/CHECKER-STUB-RESOLUTION-PLAN.md) | Stub resolution & custom typeshed — the six-step import-resolution order. The step-3 `typeshed-path` override (#271, matching Pyright's `typeshedPath` / mypy's `custom_typeshed_dir`) is **implemented**; the remaining open task is the MicroPython smoke test + verification gate. TODO at the bottom. |
