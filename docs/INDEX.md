...
| [CHECKER-STUB-RESOLUTION-PLAN.md](plans/CHECKER-STUB-RESOLUTION-PLAN.md) | Stub resolution & custom typeshed — the six-step import-resolution order; headline open task is the step-3 `typeshed-path` override (#271), designed to match Pyright's `typeshedPath` / mypy's `custom_typeshed_dir`. TODO at the bottom. |
