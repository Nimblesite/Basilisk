...
2. **Gradual adoption** — after autofixing, write per-file rule severities to
   the active project configuration so remaining errors become warnings while
...
4. The exact-file override is written into the **active project configuration
   file**. Adoption never creates a sidecar or a hidden mode.
5. From this point on, Basilisk uses `Warning` severity for those codes in those files.

### Adoption Configuration Format {#AUTOFIX-ADOPTION-FILE}

```toml
# pyproject.toml — all Basilisk configuration remains under [tool.basilisk]
...
[tool.basilisk.per-path-overrides."src/models/user.py"]
adoption = true

[tool.basilisk.per-path-overrides."src/models/user.py".rules]
"BSK-E0001" = "warning"
"BSK-E0002" = "warning"
```

`adoption = true` is provenance for the editor and auto-graduation; severity is
still ordinary per-rule configuration. Exact file paths use the same canonical
path matcher and precedence as other per-path entries. `basilisk.json` expresses
the same data in its active-source JSON shape. There is one effective config
source per root and no `.basilisk/adoptions.toml`.
...
- **Manual un-adoption.** Remove the exact-file `adoption` entry from the active
  config or use `Basilisk: Un-adopt File`.
