...
**Remaining:**
- [ ] Route the LSP **bulk scan's first pass** (and the CLI batch path) through the engine too — the scan's pre-search-paths pass still runs the direct pipeline (its results are superseded by the salsa sweep in the same startup sequence). The CLI is the conformance path, so it must prove byte-for-byte parity before flipping (and gains little: one-shot batch reuses no memos).
