...
**Remaining:**
- [ ] Eliminate the LSP analysis double-parse — the parse-error check parses outside salsa, then `resolved_module` re-parses inside it. Fold parse-error detection into the query (a richer `resolved_module` outcome, or a shared parse query) so an edit parses once. Minor, hot-path only.
...
- [ ] Content-precise cross-file invalidation — model each imported file as its own `SourceFile` input so editing an imported sibling invalidates its importers (today only re-setting `SearchPathsInput` does). This is where salsa beats the existing per-file `DashMap` cache. Large: touches the resolver + cross-module rules.
