...
**Remaining:**
- [ ] **Consume imported types inside the query, then populate `WorkspaceFiles` in the LSP.** The cross-file *mechanism* is in place but the LSP passes an **empty** registry: today the salsa query doesn't read imported files' *types* (the LSP's separate cross-module pass does), so populating it now would over-invalidate with no output change. Move cross-module type-sharing into the query (large: resolver + cross-module rules), then populate the registry so editing a dependency updates its importers' diagnostics — the payoff salsa has over the per-file `DashMap`, replacing the hand-maintained `import_graph`.
