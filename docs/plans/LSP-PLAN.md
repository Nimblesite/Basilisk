...
- [x] **The LSP is wired onto the engine** — `basilisk-lsp`'s `SalsaAnalysisEngine` (`salsa_engine.rs`) holds a persistent `BasiliskDatabase` + input handles (one `SourceFile` per file, one `ConfigInput` per root, one `SearchPathsInput`). `WorkspaceIndex::analyse_and_resolve` routes the incremental single-file path (didOpen/didChange) through `resolved_module` (navigation) + `file_diagnostics_resolved` (diagnostics) once the workspace scan has populated the search paths; the pre-scan import-free path is unchanged. **Behaviour-preserving: the full LSP e2e suite (~1000 tests incl. 71 navigation, 112 features) stays green.** The conformance path (CLI `basilisk check` → `process_file`) is untouched, so conformance is unaffected.
...
- [x] **Content-precise cross-file invalidation — mechanism** — a `WorkspaceFiles` salsa input (path → `SourceFile` map) lets `resolved_module` read the *content* of the files it imports, recording a salsa edge on each. Editing an imported file then re-runs exactly its importers' queries; editing an unrelated file does not (`editing_an_imported_file_invalidates_only_the_importer`, verified: neutering the edge-recording loop makes it fail `0 vs 1`). Adding `PartialEq` to `ResolvedModule` (prior step) is what let the module flow through salsa to make this possible.
- [x] **Cross-file that changes output — user-stub `.pyi` (live in the LSP)** — `resolved_module` re-derives a workspace-tracked user stub's API from its `SourceFile` text (`recapture_user_stub_from_source`), so editing an imported `.pyi`'s *content* updates the importer's `imports_module_attribute` diagnostics — the first cross-file edge that changes output, not just re-runs. Proven with `editing_a_user_stub_updates_the_importer_diagnostics` (verified: neutering the re-capture fails it). The LSP engine populates `WorkspaceFiles` from its open sources (rebuilt only when a file is added; steady-state editing doesn't churn it), so this is **live in the editor** — behaviour-preserving where a `SourceFile` matches disk, and the full LSP e2e suite (~1000 tests) stays green.

**Remaining:**
- [ ] **Move cross-module *type* sharing into the query.** Today the salsa query reads imported *stub* content (above) but not sibling `.py` **types** — editing `b.py`'s function signature doesn't yet update `a.py`'s type errors through salsa (the LSP's separate cross-module pass + `import_graph` still handle that). Route cross-module symbol/type resolution through a salsa query over other files' `resolved_module`, so the general dependency case updates incrementally and the hand-maintained `import_graph` can retire. Large: resolver + cross-module rules.
