...
> Spec: [CHKARCH-INCREMENTAL-SALSA](../specs/CHECKER-ARCHITECTURE-SPEC.md#CHKARCH-INCREMENTAL-SALSA). The engine is built, proven correct, and **now drives the LSP's interactive single-file analysis** (didOpen/didChange). The CLI conformance path (`basilisk check` → `process_file`) is deliberately unchanged, so this cannot affect the conformance score.
...
- [x] **The LSP's interactive single-file analysis is wired onto the engine (additive)** — `basilisk-lsp`'s `SalsaAnalysisEngine` (`salsa_engine.rs`) holds a persistent `BasiliskDatabase` + input handles (`SourceFile` per file, `ConfigInput` per root, one `SearchPathsInput` + `WorkspaceFiles`). `WorkspaceIndex::analyse_and_resolve` routes the didOpen/didChange path through `resolved_module` (navigation) + `file_diagnostics_resolved` (diagnostics) once the scan populated the search paths; the pre-scan path is unchanged. Verified genuinely exercised (instrumented: 61 engine hits in `ws_core_tests`). The full LSP e2e suite stays green; the CLI conformance path is untouched. **Honest scope:** this is *additive* — the engine runs alongside the existing `FileEntry` `DashMap`, `cross_module.rs`, `import_graph.rs`, and the non-salsa bulk scan; it replaces none of them. Its incremental win over the existing per-file cache is situational: config-only edits memo-hit `resolved_module` (skip re-parse/resolve, `config_edit_does_not_reresolve_module`), and transitive importers that don't depend on a change become memo-hits. A single-file text edit re-runs the file and its direct importers either way.
...
- [x] **Engine bookkeeping bounded on file deletion** — `SalsaAnalysisEngine::remove` (wired into `forget_file` + `set_closed`) drops a deleted file's `SourceFile` and rebuilds the registry, so the engine's maps stay consistent with the index and no cross-file edge points at a gone path (`remove_drops_a_tracked_source`). Note: the engine's `sources` is a *subset* of the workspace (only files touched by `analyse_and_resolve`), smaller than the whole-workspace `FileEntry` index; salsa 0.27 cannot reclaim an input's internal memo, so a deleted file's memo lingers until the DB is dropped — bounded, not unbounded.
- [x] **Content-precise cross-file invalidation — mechanism (checker-proven)** — a `WorkspaceFiles` salsa input (path → `SourceFile` map) lets `resolved_module` read the *content* of the files it imports, recording a salsa edge on each. Editing a tracked import re-runs exactly its importers' queries; an unrelated edit does not (`editing_an_imported_file_invalidates_only_the_importer`, verified by neutering: `0 vs 1`). For a user-stub `.pyi`, `resolved_module` re-derives the stub API from the tracked text (`recapture_user_stub_from_source`), so the edge changes *output* (`editing_a_user_stub_updates_the_importer_diagnostics`, verified by neutering). Adding `PartialEq` to `ResolvedModule` is what let the module flow through salsa.

**Remaining:**
- [ ] **Cross-file is NOT live in the LSP yet.** The mechanism above is proven only at the *checker* level. In the LSP, when a dependency changes the importer is refreshed by `set_open_refresh_dependents` → `reresolve_imports_and_recheck` — a disk-based, **non-salsa** pass — so an in-memory edit to an open user-stub `.pyi` does **not** update its importer (pinned by the characterization test `editing_open_stub_does_not_yet_refresh_importer_via_salsa`). Wiring the dependent-refresh onto the salsa query is the missing step.
- [ ] **Move cross-module *type* sharing into the query.** The query reads imported *stub* content but not sibling `.py` **types** — editing `b.py`'s signature doesn't update `a.py`'s type errors through salsa (the cross-module pass + `import_graph` still do). Route cross-module resolution through a salsa query over other files' `resolved_module`, then retire `import_graph`. This is also what would make the dependent-refresh above salsa-driven. Large: resolver + cross-module rules.
- [ ] Route the LSP **bulk scan** (and the CLI batch path) through the engine too — currently only the interactive single-file path is on salsa. The CLI is the conformance path, so it must prove byte-for-byte parity before flipping (and gains little: one-shot batch reuses no memos).
