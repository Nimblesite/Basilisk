...
**Remaining (adoption — separate, higher-risk phase):**
- [ ] Wire the LSP diagnostics path onto `file_diagnostics_resolved` (then the CLI batch path). This is the step that puts the engine on the **published-diagnostics path**, so it must prove byte-for-byte parity with today's output before flipping — it can move the conformance score if it diverges.
- [ ] Content-precise cross-file invalidation — model each imported file as its own `SourceFile` input so editing an imported sibling invalidates its importers (today only re-setting `SearchPathsInput` does). Large: touches the resolver + cross-module rules.
