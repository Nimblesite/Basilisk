...
commits, or calls a cache entry stale. Every mode without an explicit commit warns
`UNPINNED`; **Pin current** makes determinism one action away.
...
3. Cache reuse hashes ZIP bytes. Cache-off downloads, validates, and discards; it is not called hermetic. A custom miss proceeds to step 4.
...
- [ ] **Failure rules:** cached unpinned `A` plus failed Latest selects the real-body bundled ZIP and warnings, never `A`; unavailable exact pin fails closed unless the bundle SHA equals it.
- [ ] **Activation gates:** corrupt shape/tree/license metadata; add duplicate, absolute/`..`, escaping-link, over-count, and zip-bomb entries; assert rejection even with content verification off.
- [ ] **Immutable ZIP/VFS:** interrupt acquisition and mutate a cached ZIP; assert neither activates, reuse detects mutation by ZIP SHA-256, and every `.pyi` read comes from the accepted ZIP.
- [ ] **Concurrent callers:** CLI/LSP/MCP callers observe one complete identity and one atomic promotion.
...
- [ ] **Tree binding:** two archive encodings of one tree pass and any content mutation fails; a pin alone proves nothing, while verified official metadata reports its GitHub/TLS trust boundary and lack of a signed typeshed release.
- [ ] **Cache controls:** eviction re-downloads the same pin; cache-off leaves no ZIP; after verification-off cached acquisition, verification-on reruns the content gate before reporting verified.
- [ ] **Verification waived:** skip only tree hashing; safety, shape, approved-license/NOTICE checks still run; all surfaces report `UNVERIFIED` without implying verified provenance.
- [ ] **License drift:** change the approved path+SHA-256 manifest for any relevant root/nested `LICENSE*`/`NOTICE*` on Latest, pin, and mirror paths; block, report `LICENSE CHANGED`, and use bundled only under Latest rules.
- [ ] **Mirror:** known SHA downloads through `{sha}` and verifies; Latest without official metadata cannot reuse an earlier SHA and falls back loudly.
- [ ] **Status routing:** compose `UNPINNED` + fallback + `UNVERIFIED`; assert CLI uses stderr, LSP uses `showMessage`/Service Info (not `publishDiagnostics`), and MCP returns the same ordered structured warnings.
...
- [ ] **Exact commit:** configure full SHA `A`; assert exact tree/VFS bytes, later `main` movement has no effect, and unavailable `A` never substitutes another bundled SHA.
- [ ] **Pinned reuse:** validate `A`, remove the network, and assert the immutable cached ZIP remains eligible; eviction re-downloads `A` and never “expires” the pin.
...
- [ ] **Not-pinned advisory:** fresh Latest, bundled fallback, and Custom all report `UNPINNED`; only explicit `typeshed-commit` suppresses it; status never becomes a Python diagnostic.
- [ ] **Custom tree:** conflicting custom/download/bundle data resolves custom verbatim, reports user-managed terms without assuming Apache/MIT, and bypasses every other step-3 lookup.
...
- [ ] **Guard selection:** concrete target selects one version/platform branch; `All` requires validity across alternatives and never exposes a one-platform-only name (#318 regression).
- [ ] **Target environment:** cross-version checking uses that target interpreter's `site-packages`/`dist-packages`, including an explicit Python-binary override.
...
- [ ] **Package misses:** complete stub-package miss stops; `partial\n` and stub-only namespace (no `__init__.pyi`) misses continue to steps 5/6.
- [ ] **`.pyi` precedence:** place `.pyi` and `.py` for one module at the winning location; assert only `.pyi` supplies the public interface.
- [ ] **#312/#318 exports:** exact MicroPython `asyncio.sleep`, `Task`, and `run`; redundant aliases; every specified `__all__` mutation; relative/absolute stars; private exclusion; cycles; and a chain beyond the former depth cap all resolve without target-branch unions.
...
- [ ] **#289:** real `unittest.mock.Mock` plus fixtures cover special metaclass `__call__`, inherited non-`object` `__new__`/`__init__`, object fallback, binding, overloads/unions, and non-instance termination.
- [ ] **#288:** real `str.join` preserves overloads, return types, `LiteralString`, `/`, and receiver specialization/removal in hover and call checking; `.pyi` mutation proves no hand table.
...
- [ ] **Policy metadata:** `THIRD-PARTY-LICENSES`/`NOTICES` record typeshed, licenses, URL, exact SHA, derived indexes, and repackaging; any license identity/NOTICE change fails for human review.
- [ ] **MCP provenance:** structured status includes active source, full commit/tree identity, transport, license status/reference (custom may say `not supplied`), and ordered warnings.
