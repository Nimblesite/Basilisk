...
- [x] **Phase 5: Wire Into Rules** (~10 FPs) — DONE
  ...
  - [x] 5d. Full conformance suite verification — the "**18 FPs** (target < 71)" result came from an earlier in-repo harness (a miscalculation) and is superseded; the official `python/typing` scorer reports **24 false positives** (121/146 files passing, 82.9% counting errors+warnings, binary in spec-conformance mode — down from 285 FPs / 40.4% with house-style rules on). Driving FPs down remains active work.
