...
> **Correction (spec-check):** Phases 4–5 below were previously marked `[x] DONE` citing `subtyping.rs`, `SubtypeContext`, `is_subtype_with_context()`, `VarCheckContext`, `expr_inference.rs`, and `constraint_solver.rs` — **none of which exist in the codebase** (verified by grep: 0 matches; no such files). They are unchecked here to reflect reality. What exists today is the old string-based `is_assignable_to()` (`crates/basilisk-checker/src/types.rs:202`) plus two rule-local `is_subtype_of` string helpers — not the MRO/protocol engine described below. The honest 46.6% conformance (item 5d) was measured **without** this engine.

- [ ] **Phase 4: Class Hierarchy and Structural Subtyping** (~50 FPs) — planned: `crates/basilisk-checker/src/subtyping.rs` (not created)
  ...
  - [ ] 4h. Conformance verification once the engine lands — the prior "57 FPs, 0 regressions" was never real; see 5d for the honest 265-FP baseline
- [ ] **Phase 5: Wire Into Rules** (~10 FPs) — depends on Phase 4
  ...
  - [ ] Checker-side modules NOT created: `narrowing.rs`, `expr_inference.rs`, `constraint_solver.rs` (cited as done above — none exist under `crates/basilisk-checker/src/`; the resolver only collects guards for a future checker `NarrowingContext`)
