...
// This helper is not yet wired into the LSP command path, which currently
// spawns bare `uv` through OS PATH ([LSPUV-CONFIG-BINARY-RESOLUTION]).
#[must_use]
...
// Tests for [LSPUV-CONFIG-BINARY-RESOLUTION]: the cascade falls through cleanly
// for a nonexistent/empty/None config path and never panics.
#[cfg(test)]
mod tests {
    use super::*;
