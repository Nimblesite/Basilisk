...
impl WsTestFixture {
    ...
    async fn did_close(&mut self, uri: &str) -> TestResult<()> {
        self.send_json(&serde_json::json!({
            "jsonrpc": "2.0",
...
/// Create a uniquely-named temporary directory to avoid collisions when tests
/// run in parallel.  The name is `{prefix}_{nanos}` where `nanos` is the
