...
// Implements [REFACTOR-ABSTRACT] and [REFACTOR-ABSTRACT-ALGO] — resolve base
// classes and their `@abstractmethod` methods (step 1), filter out
// already-implemented ones (step 2), generate `raise NotImplementedError` stubs
// copying the signature (step 3), and insert at the end of the class body (step 4).
#[must_use]
pub(in crate::code_actions) fn implement_abstract_methods(
    ...
    Some(super::super::code_action_with_changes(
        title,
        CodeActionKind::new("refactor.rewrite.implement"),
        changes,
        false,
    ))
}
