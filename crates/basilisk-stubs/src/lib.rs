...
//! `python/typeshed` archive (downloaded over HTTPS, **never** `git clone`) or
//! the bundled snapshot is canonical; the compiled data below is the offline
//! day-one baseline it supersedes ([STUBRES-TYPESHED-BASELINE]).
//!
//! Provides:
//! - O(1) stdlib module-name recognition over the offline baseline (a `phf`
//!   set), superseded at runtime by the active source's `stdlib/VERSIONS`
...
// (`static STDLIB_MODULES: phf::Set<&'static str>`), superseded at runtime by
// the active source's `stdlib/VERSIONS` ([STUBRES-TYPESHED-BASELINE]).
...
// superseded at runtime by the active source's `stubs/<DIST>/` tree.
include!(concat!(env!("OUT_DIR"), "/stub_map.rs"));

/// Check whether a top-level module name belongs to the embedded Typeshed
/// baseline, or is Basilisk's own always-typed support module.
///
/// Extracts the root segment (before the first `.`) and checks it against the
/// matching-bundle name-set. O(1) lookup via perfect hashing. No target is
/// selected here; an active snapshot's `VERSIONS` index performs target
/// admission instead.
// Implements [STUBRES-TYPESHED] — this set is the offline baseline built by
// `build.rs` into a `phf` set for O(1) lookup; the active runtime source's
// `stdlib/VERSIONS` (downloaded archive or bundled snapshot) supersedes it. The
// stdlib list is derived data, not a maintained list.
#[must_use]
pub fn is_stdlib_module(module_name: &str) -> bool {
    let root = module_name.split('.').next().unwrap_or(module_name);
    root == "basilisk" || STDLIB_MODULES.contains(root)
}
...
///
/// Extracts the root segment (before the first `.`) and checks it against the
/// `types-<dist>` map (the active downloaded/bundled source's `stubs/<DIST>/`
/// tree, or the offline baseline when no runtime source is available). Returns
...
// Implements [STUBRES-PEP561] step 4 (stub-only packages) — names the
...
mod tests {
    ...
    fn type_provenance_from_stub_source_and_tier() {
        ...
        // (downloaded archive, bundled snapshot, or offline baseline)
