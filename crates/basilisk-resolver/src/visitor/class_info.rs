...
use super::annotations::{
    annotation_accepted_primitives, annotation_contains_readonly_expr, annotation_is_class_var,
    annotation_is_final, annotation_is_init_var, annotation_is_kw_only, annotation_requiredness,
};
    ...
    for stmt in &class.body {
        match stmt {
            ...
            Stmt::FunctionDef(func) => {
                let func_info = function_info_from(
                    bindings,
                    func,
                    Some((
                        class.name.to_string(),
                        text_range_to_span(class.name.range()),
                    )),
                );
...
fn ann_attribute(
    ...
    Some(AttributeInfo {
        ...
        required: annotation_requiredness(bindings, &ann.annotation),
        accepted_primitives: annotation_accepted_primitives(bindings, &ann.annotation),
