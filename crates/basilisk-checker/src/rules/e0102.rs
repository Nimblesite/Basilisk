...
const CODE: ErrorCode = ErrorCode {
    code: "BSK-E0102",
    docs_url: "https://basilisk-lang.org/errors/BSK-E0102",
};
...
impl Rule for TypeVarDefaultReferential {
    fn check(&self, module: &ResolvedModule, diagnostics: &mut Vec<Diagnostic>) {
        ...
        // Track TypeVar definitions in order for ordering check
        let typevar_order: Vec<&str> = module
            .typevar_calls
            .iter()
            .filter(|tv| !tv.is_typevartuple && !tv.is_paramspec)
        ...
        // Build order index for quick lookup
        let order_index: HashMap<&str, usize> = typevar_order
            .iter()
            .enumerate()
            .map(|(i, &name)| (name, i))
            .collect();
        ...
        for tv in &module.typevar_calls {
            // Only plain TypeVar can have defaults referencing other TypeVars
            ...
            // Check if default references another TypeVar
            if !typevar_names.contains(default_name.as_str()) {
                continue; // Not a TypeVar reference, handled by e0091
            }
            ...
            // 1. Ordering check: default TypeVar must appear before this TypeVar
            if let (Some(&default_pos), Some(&tv_pos)) = (
                order_index.get(default_name.as_str()),
            ...
            // 2. Bound compatibility: default's bound must be a subtype of this TypeVar's bound
            ...
            // 4. Check for outer scope references (simplified - we don't have scope info yet)
            // This would require tracking which scope each TypeVar is defined in
            // For now, we'll rely on the ordering check to catch most cases
        }
    }
}
