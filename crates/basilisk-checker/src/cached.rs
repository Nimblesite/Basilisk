...
/// `SalsaValue` bound. The derive resolves each field through salsa's dispatch
/// helper — `String`/`Option<String>` use their `SalsaValue` impls, while `Span`,
...
#[derive(Debug, Clone, PartialEq, Eq, salsa::SalsaValue, serde::Serialize, serde::Deserialize)]
