...
// ── Reversible ───────────────────────────────────────────────────────────
// A generic protocol: `Reversible[T]` requires `__reversed__(self) -> Iterator[T]`.
// The parameter matters — a class reversing to `Iterator[bytes]` does not
...
fn reversible_element_type_is_load_bearing() -> Result<(), Box<dyn std::error::Error>> {
    SpecObligation {
        spec_reason: "`__reversed__` yielding `bytes` cannot satisfy `Reversible[str]`",
...
fn container_requires_dunder_contains() -> Result<(), Box<dyn std::error::Error>> {
    SpecObligation {
        spec_reason: "`Container` is satisfied by `__contains__`, not by `__iter__`",
