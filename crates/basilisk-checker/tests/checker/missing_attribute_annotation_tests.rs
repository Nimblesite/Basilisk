//! Tests for [BSK-0005] from [CHKARCH-DIAG-MISSING]. See docs/specs/CHECKER-ARCHITECTURE-SPEC.md#CHKARCH-DIAG-MISSING
// Integration tests for BSK-0005: Missing class attribute type annotation.
...
fn scalar_literal_suppressed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "scalar literal (int) class attr should not fire BSK-0005 — type is trivially inferrable"
    );
    Ok(())
}
...
fn annotated_class_attr_no_diagnostic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "annotated class attr should not fire BSK-0005"
    );
    Ok(())
}
...
fn enum_class_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "Enum class should be exempt from BSK-0005"
    );
    Ok(())
}
...
fn protocol_class_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "Protocol class should be exempt from BSK-0005"
    );
    Ok(())
}
...
fn namedtuple_class_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "NamedTuple class should be exempt from BSK-0005"
    );
    Ok(())
}
...
";
    let diags = run_with_config(source, &annotation_rules_config())?;
    let e0005_diags: Vec<_> = diags.iter().filter(|d| d.code.code == "BSK-0005").collect();
    assert!(
        e0005_diags.is_empty(),
        "subclass overriding an annotated parent attribute should not fire BSK-0005, got: {:?}",
...
";
    ...
    assert!(
        e0005_diags.is_empty(),
        "scalar literal attrs (int, str, float, bool, bytes, None) should not fire BSK-0005, got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    Ok(())
}
...
fn scalar_string_literal_suppressed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "scalar string literal class attr should not fire BSK-0005 — type is trivially str"
    );
    Ok(())
}
...
fn scalar_bool_literal_suppressed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "scalar bool literal class attr should not fire BSK-0005 — type is trivially bool"
    );
    Ok(())
}
...
fn multiple_scalar_attrs_suppressed() -> Result<(), Box<dyn std::error::Error>> {
    let source = "class Foo:\n    a = 1\n    b = 2\n    c = 3\n";
    let diags = run_with_config(source, &annotation_rules_config())?;
    let count = diags.iter().filter(|d| d.code.code == "BSK-0005").count();
    assert_eq!(
        count, 0,
        "scalar literal attrs should not fire BSK-0005 — types are trivially inferrable"
    );
    Ok(())
}
...
";
    ...
    assert!(
        e0005_diags.is_empty(),
        "scalar literal attrs (even new ones in subclass) should not fire BSK-0005, got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    Ok(())
}
...
";
    ...
    assert!(
        !e0005_diags.is_empty(),
        "non-inferrable RHS attrs should fire BSK-0005"
    );
    Ok(())
}

// ---------------------------------------------------------------------------
// Regression tests: scalar literals must NEVER produce false-positive BSK-0005.
...
/// NONE of these should fire BSK-0005.
#[test]
...
";
    ...
    assert!(
        e0005_diags.is_empty(),
        "scalar literal attrs in animal hierarchy must not fire BSK-0005 (regression), got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    Ok(())
}
...
/// override with new scalar values. No BSK-0005 anywhere.
#[test]
...
";
    ...
    assert!(
        e0005_diags.is_empty(),
        "config override pattern with scalar literals must not fire BSK-0005 (regression), got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    Ok(())
}
...
";
    ...
    assert!(
        e0005_diags.is_empty(),
        "standalone classes with scalar literals must not fire BSK-0005 (regression), got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    Ok(())
}
...
fn regression_each_scalar_type_individually() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (type_name, source) in cases {
        ...
        assert!(
            e0005_diags.is_empty(),
            "{type_name} literal must not fire BSK-0005 (regression), got: {:?}",
            e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
        );
    }
    Ok(())
...
";
    ...
    assert_eq!(
        e0005_diags.len(),
        1,
        "only the non-inferrable attr should fire BSK-0005, scalars must be suppressed, got: {:?}",
        e0005_diags.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    assert!(
        e0005_diags[0].message.contains("unknown"),
        "BSK-0005 should fire for `unknown`, got: {}",
...
fn type_alias_type_in_class_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "TypeAliasType alias in a class body must not fire BSK-0005, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
...
fn tuple_literal_match_args_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "tuple-literal class attrs must not fire BSK-0005, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
...
fn pep695_type_param_attr_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-0005"),
        "PEP 695 type-param-named attr must not fire BSK-0005, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
...
fn empty_collection_still_fires() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        codes(&diags).contains(&"BSK-0005"),
        "empty-list class attr must still fire BSK-0005 (element type unknown), got: {:?}",
        codes(&diags)
    );
    Ok(())
}
