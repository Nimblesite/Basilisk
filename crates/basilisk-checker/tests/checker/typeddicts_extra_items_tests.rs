...
#[test]
fn closed_inherited_extra_key_fires() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn closed_inherited_no_new_key_ok() -> Result<(), Box<dyn std::error::Error>> {
    let source = "from typing import TypedDict\nclass BaseMovie(TypedDict, closed=True):\n    name: str\nclass MovieA(BaseMovie):\n    pass\n";
    let diags = run(source)?;
    assert!(
        !codes(&diags).contains(&"typeddicts_extra_items"),
...
#[test]
fn inherited_required_extra_item_fires() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn inherited_notrequired_inconsistent_fires() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn inherited_extra_item_consistent_ok() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn readonly_extra_item_not_assignable_fires() -> Result<(), Box<dyn std::error::Error>> {
