//! Tests for [BSK-W0040] from [CHKARCH-DIAG-IMMUTABILITY] / [TYPEINF-FUNC-LAMBDA]. See docs/specs/CHECKER-ARCHITECTURE-SPEC.md#CHKARCH-DIAG-IMMUTABILITY
...
fn unannotated_lambda_fires() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ...
        "unannotated lambda should fire BSK-W0040, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
...
fn annotated_lambda_no_diagnostic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !codes(&diags).contains(&"BSK-W0040"),
        "annotated lambda should not fire BSK-W0040"
    );
    Ok(())
}
...
fn lambda_is_warning_not_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(w0040.is_some(), "should fire BSK-W0040");
    let Some(diag) = w0040 else {
        return Err("BSK-W0040 diagnostic missing after assertion".into());
    };
    assert_eq!(
        diag.severity,
        basilisk_checker::Severity::Warning,
        "BSK-W0040 should be a warning"
    );
    Ok(())
}
...
fn enum_body_lambda_exempt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // annotating them is discouraged, so BSK-W0040 must not fire
    // (conformance enums_members.py).
    let source = "from enum import Enum\nclass Color(Enum):\n    RED = 1\n    converter = lambda x: str(x)\n";
    let diags = run_with_config(source, &annotation_rules_config())?;
    assert!(
        !codes(&diags).contains(&"BSK-W0040"),
        "lambda in an enum body must not fire BSK-W0040, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
...
fn non_enum_class_lambda_still_fires() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        codes(&diags).contains(&"BSK-W0040"),
        "lambda in a non-enum class must still fire BSK-W0040, got: {:?}",
        codes(&diags)
    );
    Ok(())
}
