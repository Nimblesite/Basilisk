...
fn typeshed_path_full_lifecycle_through_pyproject() {
    ...
    // ── Interaction 1: NO typeshed-path → the bundled snapshot resolves both
    // stdlib modules, so the project is clean. ──
    write(
        &dir,
        "pyproject.toml",
        "[project]\nname = \"x\"\nversion = \"0.1.0\"\n",
    ...
    // ── Interaction 5: remove typeshed-path entirely → the bundled snapshot
    // becomes active again, so the project is clean once more. ──
    write(
        &dir,
        "pyproject.toml",
        "[project]\nname = \"x\"\nversion = \"0.1.0\"\n",
