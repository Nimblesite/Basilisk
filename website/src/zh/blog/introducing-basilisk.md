...
在现有代码库上采用 Basilisk 确实需要工作——但这是暴露真实错误的工作。每个 BSK-E0001 都是一个从未定义类型契约的函数。每个 match_exhaustiveness 都是一个被静默忽略未处理情况的 `match` 语句。Basilisk 报告的错误不是误报——它们是类型系统未被使用的地方。
