...
| PEP 符合性¹ | 当前 {{ conformance.scorePct }}%（目标 →100%） | ~99% | ~58% | 早期 alpha | ~86% |
...
**Basilisk 尚不是最佳选择的地方：**
- PEP 符合性：Basilisk 当前通过官方符合性套件的 {{ conformance.scorePct }}%（{{ conformance.pass }}/{{ conformance.total }}，错误加警告，最严格评分），仍有 {{ conformance.fp }} 处误报和 {{ conformance.missed }} 处遗漏的必需错误正在被压低。Pyright 今天覆盖远更多边缘情况。Basilisk 的目标是 100%；还未达到。参见[我们如何衡量符合性](/zh/docs/conformance/)。
...
诚实的建议：开始新 Python 项目的团队应该使用 Basilisk，从第一天起就受益于严格执行——尤其是如果他们跨多款编辑器工作。在现有良好类型化代码库上从 Pyright 迁移的团队应该在符合性趋近同等水平时进行评估。
