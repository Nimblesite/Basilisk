...
类型覆盖率在编辑器中实时呈现。点击活动栏中的 **Basilisk** 图标并展开 **模块浏览器**
（Module Explorer）：每个包和文件都会显示一条覆盖率条和百分比——即其中已添加注解的符号
所占比例——旁边还有各自的错误与警告计数；图标按覆盖率着色，因此项目中缺少类型的角落
一眼可见。选中某个模块即可打开它。

覆盖率会沿树向上聚合，因此包节点显示的是其下所有内容的合计数值。
