...
1. **Inventory website claims** — Identify every feature, capability, diagnostic code, supported construct, and technical detail mentioned on the website.
2. **Read the codebase** — Examine the relevant source files (checker rules, LSP handlers, diagnostic definitions, supported features) to determine what the code actually implements today.
3. **Fix inaccuracies** — For each claim on the website:
...
4. **Verify diagnostic codes** — Ensure any diagnostic codes (`BSK-E####` / `BSK-W####`) listed on the website match the codes defined in the codebase.
5. **Check version/compatibility claims** — Confirm Python version targets, supported platforms, and dependency information match `Cargo.toml`, CI config, and docs.
