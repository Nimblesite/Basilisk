...
const OVERVIEW_SECTION = `
  <section id="overview-section" data-section="overview" aria-labelledby="overview-title" hidden>
    ...
    <div id="overview-grid">
      ...
      <article class="card wide" data-accent="orange"><h3>Choose a policy preset</h3><p>Presets are named, LSP-owned transactions—not hidden modes. Preview shows the explicit rules and severities that will be persisted.</p><div id="preset-list" class="action-row"></div><div class="action-row"><button type="button" class="secondary" data-action="disable-all">Disable all…</button></div></article>
    </div>
  </section>`;
...
const ADOPTION_SECTION = `
  <section id="adoption-section" data-section="adoption" aria-labelledby="adoption-title" hidden>
    ...
    <div id="adoption-grid">
      <article class="card" data-accent="orange"><h3>1 · Set the target</h3><p>Choose a server-defined preset. The Strict preset turns on the complete live catalog.</p><div id="adoption-presets" class="action-row"></div></article>
      <article class="card" data-accent="sky"><h3>2 · Take the free wins</h3><p>Ask the language server to apply every currently available safe fix in this workspace. Unsafe fixes are never included.</p><button id="fix-safe-button" type="button" class="primary" data-action="fix-safe">Apply safe fixes</button></article>
      <article class="card"><h3>3 · Decide what cannot move yet</h3><p>Preview disabling every rule whose current violations have no safe fix. This hides current and future diagnostics for those rules until you re-enable them; nothing is silently disabled.</p><div class="action-row"><button type="button" class="danger" data-action="disable-unfixable">Preview disabling unfixable rules…</button><button type="button" class="secondary" data-action="show-unfixable">Review first</button></div></article>
...
const PATHS_SECTION = `
  <section id="paths-section" data-section="paths" aria-labelledby="paths-title" hidden>
    <div class="section-heading"><h2 id="paths-title">Path overrides</h2><p>Inspect every persisted exception, remove it as one reviewed change, or apply selected rules to a project-relative pattern.</p></div>
    <div id="project-grid">
      <div id="path-override-list" class="wide" aria-label="Existing path overrides"></div>
      <form id="path-form" class="card wide">
        ...
        <label for="path-setting">Severity</label><select id="path-setting"><option value="Native">Native severity</option><option value="Error">Error</option><option value="Warning">Warning</option><option value="Info">Info</option><option value="Disabled">Disabled</option><option value="Inherit">Inherited · reset</option></select>
...
const OVERLAYS = `
  ...
  <dialog id="preview-dialog" aria-labelledby="preview-title"><header><h2 id="preview-title">Review exact impact</h2></header><div id="preview-body"><div id="impact-grid"></div><p id="expanded-rules"></p><h3>Exact resolved changes</h3><div id="preview-changes"></div><div id="preview-problems"></div></div><footer><button type="button" class="secondary" data-action="close-preview">Cancel</button><button type="button" class="primary" data-action="apply-preview">Apply changes</button></footer></dialog>
