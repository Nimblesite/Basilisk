...
⚠️ There is ONE conformance path and it is the REAL upstream harness, run FRESH on every CI run: `conformance/run_conformance.py` clones `python/typing@main` and runs the suite's OWN unmodified `conformance/src/main.py --only-run basilisk` (its `type_checker.py` already ships the official `BasiliskTypeChecker`) against the compiled binary. **NO** vendored calculator, **NO** reimplemented/injected adapter of ours, **NO** cached-fixtures fallback, **NO** committed results substituting for a live run. `conformance_status.csv` is GENERATED from the harness's own `results/basilisk/*.toml`. A build in which that official check did not actually run against a freshly cloned suite is a **BUILD FAILURE** — never re-introduce a home-grown scorer. ⚠️
