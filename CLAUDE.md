...
- **Robustness ratchets, always.** The pristine fixture regression and `make mutation-conformance` must improve from their audited baselines. A semantics-preserving rename or formatting change must not change a verdict. If removing fitted code lowers the raw suite number, publish the lower number once it is trustworthy.
