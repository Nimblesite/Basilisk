...
# ── BSK-0003: unannotated empty collections at module scope ─────────────────
_schema_cache = {}  # BSK-0003: empty dict
_transform_registry = []  # BSK-0003: empty list


# ── BSK-0001/0002: untyped ETL stage functions ─────────────────────────────
def extract(source, options):  # BSK-0001: source, options untyped
    records = source.read_all()
    return records  # BSK-0002: no return type


def transform(records, schema, strict):  # BSK-0001: three untyped params
    result = []
    for row in records:
        result.append(row)
    return result  # BSK-0002: no return type


def load(records, destination):  # BSK-0001: records, destination untyped
    destination.write(records)
    # implicit return None — no annotation # BSK-0002: no return type
...
@overload
def read_source(path) -> list[dict[str, str]]: ...  # BSK-0001: path untyped


@overload
def read_source(
    path,
) -> list[dict[str, str]]: ...  # BSK-0001 + overloads_consistency: duplicate
...
# ── BSK-0025: override not decorated ───────────────────────────────────────
class BaseWriter:
...
class ParquetWriter(BaseWriter):
    def flush(self, data: list[bytes]) -> int:  # BSK-0025: missing @override
        return len(data) * 2
